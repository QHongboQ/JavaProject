...
public class zyBook_6_7_2
{
    ...
    {
        int writeIndex = 0; // Index to write the next even number

        for (int i = 0; i < size; i++) {
            // Check if the number is even
            if (values[i] % 2 == 0) {
                // If even, write it to the writeIndex and increment writeIndex
                values[writeIndex] = values[i];
                writeIndex++;
            }
        }

        // Return the new size of the array, which is the last index we wrote to
        return writeIndex;
    }
